...
Shader "Custom/Gravitation Lensing Shader"
{
		Properties{
			...
			SubShader{
				Pass {
					...
					float4 frag(v2f i) : COLOR
					{
						...
						//Rad is schwarzschild radius
						//Distance is distance from the camera to the "len"
						//rad is distance from the "len" to the 2d points on the screen
						//caluclation seems just a bit of off but for some reason it "works"
						float deformation = 1 / pow(rad*pow(_Distance, 0.5), 3)*_Rad * 2;
